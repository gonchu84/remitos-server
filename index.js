...
// === Autorización: dos roles por PIN ===
const ADMIN_PIN     = process.env.ADMIN_PIN     || "1234"; // Admin (central)
const RECEIVER_PIN  = process.env.RECEIVER_PIN  || "9999"; // Recepción (sucursales)
function requireAdmin(req, res) {
  const pin = String(req.query.pin || "");
  if (pin !== ADMIN_PIN) { res.status(401).json({ error: "PIN admin inválido" }); return false; }
  return true;
}
function requireReceiver(req, res) {
  const pin = String(req.query.pin || "");
  if (pin !== RECEIVER_PIN && pin !== ADMIN_PIN) { // admin también puede recepcionar
    res.status(401).json({ error: "PIN recepción inválido" });
    return false;
  }
  return true;
}

// Limpia un teléfono a formato wa.me (solo dígitos)
...
<body>
  ...
  <div class="card">
    ...
    <form method="POST" action="/admin/products-xlsx?pin=${encodeURIComponent(ADMIN_PIN)}" enctype="multipart/form-data">
...
app.post("/admin/products-xlsx", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    res.redirect(`/admin/products-xlsx?pin=${encodeURIComponent(ADMIN_PIN)}&msg=${encodeURIComponent(msg)}`);
  } catch (e) {
    console.error(e);
...
// ===== Recepción protegida por RECEIVER_PIN (o ADMIN_PIN) =====
...
// ======= RECEPCIÓN /r/:id (HTML) con búsqueda y ENTER + PIN local =======
...
"      <input id=\"find\" class=\"input\" placeholder=\"Buscar; Enter suma al primero visible\" />" +
...
"  // ==== PIN recepción guardado en localStorage ====" +
"  function getRecvPin(){ let p=localStorage.getItem('recv_pin')||''; if(!p){ p=prompt('Ingresá PIN de recepción:')||''; localStorage.setItem('recv_pin',p);} return p; }" +
"  function qp(url){ const pin=getRecvPin(); return url + (url.includes('?')?'&':'?') + 'pin=' + encodeURIComponent(pin); }" +
...
"    const rr = await fetch(qp(BASE + '/remitos/' + RID + '/scan'), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ code: t }) });" +
...
"      else if(j.error && j.error.includes('PIN')){ localStorage.removeItem('recv_pin'); alert('PIN inválido. Probá de nuevo.'); return; }" +
...
"    if(e.key!=='Enter') return; const fq=fold(e.target.value);" +
"    const rows = Array.from(document.querySelectorAll('#tb tr'));" +
"    const matchRows = rows.filter(tr=>{ const text=(tr.getAttribute('data-desc')||'')+' '+(tr.getAttribute('data-codes')||''); return !fq || fold(text).includes(fq); });" +
"    if(matchRows.length===0) return;" +
"    const first = matchRows[0]; const idx = rows.indexOf(first);" +
"    const rr = await fetch(qp(BASE + '/remitos/' + RID + '/scan'), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ code: (first.getAttribute('data-codes')||'').split(' ')[0] || first.getAttribute('data-desc') }) });" +
"    if(rr.ok){ const j=await rr.json(); const td=document.getElementById('rcv_'+j.item.index); if(td) td.textContent=j.item.received; const st=document.getElementById('st'); st.textContent=String(j.status||'').toUpperCase(); st.className='pill '+cls(j.status||'pendiente'); }" +
"  });" +

"  async function closeOk(){ const note = prompt('Observaciones (opcional):','')||'';" +
"    const rr=await fetch(qp(BASE + '/remitos/' + RID + '/close'), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'ok', note }) });" +
"    if(!rr.ok){ const j=await rr.json().catch(()=>({})); if(j.error&&j.error.includes('PIN')){localStorage.removeItem('recv_pin'); alert('PIN inválido'); return;} alert(j.error||'No se pudo cerrar en OK'); return; } showMsg('Remito cerrado en OK'); reload(); }" +

"  async function sendDiff(){ const note = prompt('Describí las diferencias:','')||'';" +
"    const rr = await fetch(qp(BASE + '/remitos/' + RID + '/close'), { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'diferencias', note }) });" +
"    if(!rr.ok){ const j=await rr.json().catch(()=>({})); if(j.error&&j.error.includes('PIN')){localStorage.removeItem('recv_pin'); alert('PIN inválido'); return;} alert('No se pudo enviar diferencias'); return; } showMsg('Diferencias enviadas'); reload(); }" +
"</script>" +
"</body></html>";
...
function generateRemitoPDF(remito, outPath) {
  ...
  if (remito.note && String(remito.note).trim()) {
    doc.moveDown(1);
    doc.fillColor("#111").fontSize(12).text("Observaciones:", { underline: true });
    doc.moveDown(0.2);
    doc.fillColor("#444").fontSize(11).text(String(remito.note));
  }
