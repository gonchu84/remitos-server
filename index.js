...
/* === Base pública para armar URLs absolutas (Render) ===
   Definila en Render → Environment Variables:
...
app.get("/products", (req, res) => {
  ...
  if (!q)
    return res.json(
      (db.products || []).map((p) => ({ id: p.id, description: p.description, codes: p.codes || [] }))
    );
  const qn = norm(q);
...
app.get("/admin/products-xlsx", (req, res) => {
  ...
  res.send(
    "<html><head><meta charset=\"utf-8\"><title>Importar productos (.xlsx)</title>" +
      "<style>body{font-family:system-ui;padding:20px}input,button{padding:8px}.card{border:1px solid #e5e7eb;border-radius:10px;padding:12px;max-width:720px}pre{background:#f8fafc;border:1px solid #e5e7eb;border-radius:8px;padding:10px;overflow:auto}</style></head><body>" +
      "<h2>Importar productos (.xlsx)</h2>" +
      "<div class=\"card\">" +
      "<p>Formato columnas (primera hoja):</p>" +
      "<ul><li><b>A</b>: código1</li><li><b>B</b>: código2</li><li><b>C</b>: código3</li><li><b>D</b>: descripción</li></ul>" +
      "<form method=\"POST\" action=\"/admin/products-xlsx\" enctype=\"multipart/form-data\">" +
      "<input type=\"file\" name=\"file\" accept=\".xlsx,.xls\" required /> " +
      "<button type=\"submit\">Subir</button></form></div>" +
...
app.post("/remitos", (req, res) => {
  ...
  // ===== WhatsApp (si hay teléfono de sucursal) =====
  let wa = null;
  if (branchPhone) {
    const text = [
...
// Escaneo: +1 al ítem (permite superar lo enviado)
app.post("/remitos/:id/scan", (req, res) => {
  ...
  it.received = (parseInt(it.received, 10) || 0) + 1; // NO cap
...
// Nuevo: delta ±1 o set manual de recibido
app.post("/remitos/:id/recv", (req, res) => {
  const db = loadDB();
  const id = Number(req.params.id);
  const { index, delta, received } = req.body || {};
  const remito = (db.remitos || []).find((x) => x.id === id);
  if (!remito) return res.status(404).json({ error: "No existe" });

  const idx = Number(index);
  if (isNaN(idx) || idx < 0 || idx >= (remito.items || []).length) {
    return res.status(400).json({ error: "Ítem fuera de rango" });
  }

  const it = remito.items[idx];
  const cur = parseInt(it.received, 10) || 0;

  let next = cur;
  if (typeof received === "number") {
    next = Math.max(0, parseInt(received, 10) || 0);
  } else if (typeof delta === "number") {
    next = Math.max(0, cur + parseInt(delta, 10));
  }
...
// Cerrar: guarda observaciones y estado
app.post("/remitos/:id/close", (req, res) => {
  ...
  remito.note = String(note || ""); // guarda observaciones
  saveDB(db);
  res.json({ ok: true, status: remito.status, remito });
});

// ======= RECEPCIÓN /r/:id (HTML) con búsqueda + sumar/restar + WA al cerrar =======
app.get("/r/:id", (req, res) => {
  ...
  // WhatsApp ya pre-armado (si existe) o básico
  const waHref = r.wa ? r.wa : "https://wa.me/?text=" + encodeURIComponent("Remito " + r.numero);

  // Filas de la tabla (incluye códigos para búsqueda)
  const rowsHtml = (r.items || [])
    .map((it, i) => {
      ...
      return (
        ...
        "<td>" +
        desc +
        (codes ? "<div style='color:#64748b;font-size:12px'>" + codes + "</div>" : "") +
        "</td>" +
        "<td>" +
        qty +
        "</td>" +
        ...
        "</td>" +
        "<td>" +
        '<button class="btn secondary mini" onclick="addDelta(' +
        i +
        ', -1)">-1</button> ' +
        '<button class="btn secondary mini" onclick="addDelta(' +
        i +
        ', 1)">+1</button> ' +
        '<button class="btn secondary mini" onclick="setManual(' +
        i +
        ')">Editar</button>' +
        "</td>" +
        "</tr>"
  ...
  const html =
    ...
    "td,th{border-top:1px solid var(--line);padding:8px;vertical-align:top}" +
    ...
    "</div>" +
    "      <div><b>Destino:</b> " +
    (r.branch && r.branch.name ? r.branch.name : "") +
    "</div>" +
    '      <div><b>Estado:</b> <span id="st" class="pill ' +
    (r.status === "ok" ? "ok" : r.status === "diferencias" ? "diff" : "pend") +
    '">' +
    ...
    '      <input id="find" class="input" placeholder="Escribí para encontrar y ENTER para sumar al primero visible" />' +
    '      <button class="btn secondary" onclick="reload()">Actualizar</button>' +
    '      <a class="btn secondary" href="' +
    waHref +
    '" target="_blank">WhatsApp (link remito)</a>' +
    "    </div>" +
    '    <div style="margin-top:12px">' +
    "      <table>" +
    "<thead><tr><th>#</th><th>Descripción</th><th>Enviado</th><th>Recibido</th><th>Acciones</th></tr></thead>" +
    '        <tbody id="tb">' +
    rowsHtml +
    "        </tbody>" +
    "      </table>" +
    ...
    '      <input id="obs" class="input" placeholder="Observaciones (opcional)" />' +
    "    </div>" +
    ...
    r.pdf +
    "</a></div>" +
    '    <iframe title="PDF" src="' +
    r.pdf +
    '" style="width:100%;height:520px;border:1px solid var(--line);border-radius:10px;margin-top:6px"></iframe>' +
    "  </div>" +
    "</div>" +
    "<script>" +
    ...
    r.id +
    ";" +
    "  const WA_ADMIN = '5491150976156';" + // número destino fijo
    ...
    "  findEl.addEventListener('input', (e)=>{ const fq=fold(e.target.value);" +
    ...
    "  });" +
    "  findEl.addEventListener('keydown', async (e)=>{" +
    "    if(e.key!=='Enter') return;" +
    "    const first = Array.from(document.querySelectorAll('#tb tr')).find(tr=>tr.style.display!=='none');" +
    "    if(!first) return;" +
    "    const idx = Number(first.getAttribute('data-index'));" +
    "    await addDelta(idx, 1);" +
    "  });" +
    "  async function addDelta(idx, delta){ const rr = await fetch(BASE + '/remitos/' + RID + '/recv', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ index: idx, delta }) });" +
    "    if(!rr.ok){ alert('No se pudo actualizar'); return; } const j=await rr.json();" +
    "    const td=document.getElementById('rcv_'+idx); if(td) td.textContent=j.item.received;" +
    "    const st=document.getElementById('st'); st.textContent=String(j.status||'').toUpperCase(); st.className='pill '+cls(j.status||'pendiente');" +
    "  }" +
    "  async function setManual(idx){ const cur = Number((document.getElementById('rcv_'+idx)?.textContent)||'0')||0; const v = prompt('Nuevo recibido:', cur);" +
    "    if(v==null) return; const val = Math.max(0, parseInt(v,10)||0);" +
    "    const rr = await fetch(BASE + '/remitos/' + RID + '/recv', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ index: idx, received: val }) });" +
    "    if(!rr.ok){ alert('No se pudo actualizar'); return; } const j=await rr.json();" +
    "    const td=document.getElementById('rcv_'+idx); if(td) td.textContent=j.item.received;" +
    "    const st=document.getElementById('st'); st.textContent=String(j.status||'').toUpperCase(); st.className='pill '+cls(j.status||'pendiente');" +
    "  }" +
    "  function buildSummaryText(data, type, note){ " +
    "    const lines = [];" +
    "    lines.push((data.branch && data.branch.name ? data.branch.name : 'Sucursal'));" +
    "    (data.items||[]).forEach(it=>{ lines.push(`${it.description} ${it.qty} / ${it.received||0}`); });" +
    "    if(type==='ok'){ lines.push(''); lines.push('Remito OK'); }" +
    "    else { lines.push(''); lines.push('Remito con las siguientes Observaciones'); if(note) lines.push(note); }" +
    "    return lines.join('\\n');" +
    "  }" +
    "  async function closeOk(){ const note = document.getElementById('obs').value.trim();" +
    "    const rr=await fetch(BASE + '/remitos/' + RID + '/close', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'ok', note }) });" +
    "    if(!rr.ok){ const j=await rr.json().catch(()=>({})); alert(j.error||'No se pudo cerrar en OK'); return; }" +
    "    const data = await (await fetch(BASE + '/remitos/' + RID)).json();" +
    "    const msg = buildSummaryText(data, 'ok', note);" +
    "    const url = `https://wa.me/5491150976156?text=${encodeURIComponent(msg)}`;" +
    "    window.open(url, '_blank');" +
    "    showMsg('Remito cerrado en OK'); reload();" +
    "  }" +
    "  async function sendDiff(){ const note = document.getElementById('obs').value.trim();" +
    ...
    "    const data = await (await fetch(BASE + '/remitos/' + RID)).json();" +
    "    const msg = buildSummaryText(data, 'diff', note);" +
    "    const url = `https://wa.me/5491150976156?text=${encodeURIComponent(msg)}`;" +
    "    window.open(url, '_blank');" +
...
function generateRemitoPDF(remito, outPath) {
  ...
  if (remito.note && String(remito.note).trim()) {
    doc.moveDown(0.8);
    doc.fontSize(10).fillColor("#444").text("Observaciones:");
    doc.fontSize(11).fillColor("#111").text(String(remito.note), { width: 520 });
  }
